...
using C_Sharp_Playground.Tests;
...
namespace C_Sharp_Playground
{
    public static class SeleniumExtensions
    {
        ...
        /// <remarks>
        /// extension метод, обязательно статический, обязательно в статическом классе. Перед первым аргументом обязательное ключевое слово this
        /// тут он нужен для того чтоб можно было переходить по условным ссылкам
        /// </remarks>
        public static IWebDriver GoToRelativeUrl(this IWebDriver driver, string relativeUrl)
        {
            ...
            //for (int i = 0; i < 50; i++)
            //{
            //    if (!driver.FindElements(By.Id("loader")).Any())
            //    {
            //        break;
            //    }
            //    Thread.Sleep(100);
        ...
        public static void SelectInRegularComboboxDown(this IWebDriver driver, By locator, int elementPosition)
        {
            ...
            var el = driver.FindElement(locator);
            var downSequence = Enumerable.Repeat(Keys.Down, elementPosition + 1);// создается список повторяющихся элементов. Keys.Down- элемент списка
        ...
        {
            var el = driver.FindElement(locator);
            var upSequence = Enumerable.Repeat(Keys.Up, elementPosition + 1);// создается список повторяющихся элементов. Keys.Up- элемент списка
        ...
        public static void WaitForAjax(this IWebDriver driver, int timeout = 10000) 
        {
            var wait = new WebDriverWait(driver, TimeSpan.FromMilliseconds(timeout));
        ...
        {
            driver.WaitUntilNotVisible(By.CssSelector("#loader-fade"));
            return driver;
        }
